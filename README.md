...
Enumeration into related pterodactyl files on the server returned and environment file containing MySQL credentials, where we were able to extract the encrypted password hash of the phileasfogg3 user. We then used John the Ripper to successfully recover the plaintext password.
...
| :-------------- | :----------- | :------------------------------------------------------------- |
| CVE-2025-49132 | Critical: 10 | Unauthenticated Remote Code Execution on a remote server running Pterodactyl. |
